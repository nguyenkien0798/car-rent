...
export default function Orders({
  ...
  const totalPage = listOrder.pagination.total
    ? Math.ceil(listOrder.pagination.total / 10)
    : 0;
      ...
      {listDataOrders ? (
        <>
          <div className="mb-8 flex justify-center">
            <p className="xs:text-[24px] sm:text-[32px] font-bold">
              {dictionary.orders.ordersList}
            </p>
          </div>
              ...
              <TableHead>
                <TableRow>
                  <StyledTableCell sx={{ maxWidth: "200px" }}>
                    {dictionary.orders.code}
                  </StyledTableCell>
                  <StyledTableCell sx={{ maxWidth: "250px" }} align="center">
                  ...
                  <StyledTableCell align="center">
                    {dictionary.orders.status}
                  </StyledTableCell>
                  <StyledTableCell align="center">
                    {dictionary.orders.action}
                  </StyledTableCell>
                </TableRow>
              </TableHead>
              <TableBody>
                {listDataOrders.map((row, index) => (
                  <StyledTableRow key={row.id + index} hover>
                    ...
                    <StyledTableCell align="center">
                      <p
                        onClick={() =>
                          router.push(`/${params}/orders/${row.id}`)
                        }
                        className="text-[16px] underline cursor-pointer"
                      >
                        {dictionary.orders.detail}
                      </p>
                    </StyledTableCell>
                  </StyledTableRow>
                ))}
