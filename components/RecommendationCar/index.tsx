...
export default function RecommendationCar({
  ...
  const [listRecommendCar, setListRecommendCar] = React.useState<DataCardCar[]>(
    listCar.items,
  );
  const [totalListCar, setTotalListCar] = React.useState<number>(
    listCar.pagination.total,
  );
  ...
  const getData = async () => {
    const listData = await customFetch<{ data: ListCar }>("/v1/cars", {
      query: { filter: "recommended", limit: 8, page },
      lang: params.lang,
    });
